...
def main():
    users = {
        '1' : {
            ...
            'department' : {'name':'economic'},
            'date_joined' : '2019-07-01 10:00:00'
        },
        '2' : {
            'username' : 'Belousov',
            'email' : 'belousov@mail.com',
            'department' : {'name':'quality engineer'},
            'date_joined' : '2019-07-01 10:00:00'
        },
        '3' : {
            'username' : 'Alexeevich',
    ...
    class Departments:
        def GET(self, name=None):
                ...
                data,data2 = [],[]
                for idd in users:
                    if users[idd]['department']['name'] not in data:
                        data.append(users[idd]['department']['name'])
                if name == None:
                    return ('%s' % data)
                else:
